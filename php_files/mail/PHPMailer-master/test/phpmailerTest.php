...
class PHPMailerTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        if (file_exists('testbootstrap.php')) {
            require_once 'testbootstrap.php'; //Overrides go in here
        }
    ...
    public function testMultiplePlainFileAttachment()
    {
        ...
        if (!$this->Mail->addAttachment(realpath(self::require_once_DIR . 'examples/images/phpmailer.png'))) {
...
EOT;
    ...
    public function testHtmlIso8859()
    {
        ...
        $this->Mail->msgHTML(
            ...
            realpath(self::require_once_DIR . 'examples')
        );
        $this->buildBody();
        $this->assertTrue(
...
EOT;
        $this->Mail->addEmbeddedImage(
            realpath(self::require_once_DIR . 'examples/images/phpmailer.png'),
            'my-attach',
            'phpmailer.png',
            'base64',
            'image/png'
...
EOT;
    ...
    {
        $message = file_get_contents(realpath(self::require_once_DIR . 'examples/contentsutf8.html'));
        $this->Mail->CharSet = 'utf-8';
        $this->Mail->Body = '';
        $this->Mail->AltBody = '';
        //Uses internal HTML to text conversion
        $this->Mail->msgHTML($message, realpath(self::require_once_DIR . 'examples'));
        ...
        $this->Mail->AltBody = '';
        $this->Mail->msgHTML($message, realpath(self::require_once_DIR . 'examples'), function ($html) {
    ...
    public function testHTMLAttachment()
    {
        ...
        if (!$this->Mail->addAttachment(
            realpath(self::require_once_DIR . 'examples/images/phpmailer_mini.png'), 'phpmailer_mini.png')
        ) {
            $this->assertTrue(false, $this->Mail->ErrorInfo);
            return;
        }
    ...
    public function testHTMLStringEmbedNoName()
    {
        ...
        if (!$this->Mail->addStringEmbeddedImage(
            file_get_contents(realpath(self::require_once_DIR . 'examples/images/phpmailer_mini.png')),
    ...
    public function testHTMLMultiAttachment()
    {
        ...
        if (!$this->Mail->addAttachment(realpath(self::require_once_DIR . 'examples/images/phpmailer_mini.png'), 'phpmailer_mini.png')) {
            $this->assertTrue(false, $this->Mail->ErrorInfo);
            return;
        }

        if (!$this->Mail->addAttachment(realpath(self::require_once_DIR . 'examples/images/phpmailer.png'), 'phpmailer.png')) {
            $this->assertTrue(false, $this->Mail->ErrorInfo);
            return;
        }
    ...
    public function testIcal()
    {
        ...
        require_once realpath(self::require_once_DIR . 'extras/EasyPeasyICS.php');
